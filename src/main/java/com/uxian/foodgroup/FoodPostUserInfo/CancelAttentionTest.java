...
public class CancelAttentionTest {
	...
	private static final Logger log = Logger.getLogger(CancelAttentionTest.class);
	
	@DataProvider(name = "getData")
	private Object[][] getData() {
		return new Object[][] {{"输入已关注的用户ID", addAttentionUserId, "Success" }, {"输入未关注的用户ID", addAttentionUserId, "Exception" } };
	}
	
	@DataProvider(name = "getData_unnormal")
	private Object[][] getData_unnormal() {
		return new Object[][] {{"输入不存在的用户ID", foodUserId, deviceType, addAttentionUserId, "wrong", "出现错误。" },
				{"用户ID不输入", foodUserId, deviceType, addAttentionUserId, "", "出现错误。" },
	...
	public void setUp() throws UnsupportedEncodingException {
		...
		actionUrl = "/api/FoodPostUserInfo/CancelAttentionUser";
